    ...
    class SeleniumWebAutomationFrameworkInstance : IWebAutomationFrameworkInstance
    {
        ...
        public Task DragDropAsync(IDomElement from, int? fromOffsetX, int? fromOffsetY, IDomElement to, int? toOffsetX, int? toOffsetY)
        {
            ...
            Actions
                .MoveToElement(
                    nativeFromElement,
                    (int)Math.Ceiling(fromOffsetX ?? from.BoundingClientRectangle.RelativeCenter.X),
                    (int)Math.Ceiling(fromOffsetY ?? from.BoundingClientRectangle.RelativeCenter.Y))
                .ClickAndHold()
                .MoveToElement(
                    nativeToElement,
                    (int)Math.Ceiling(toOffsetX ?? to.BoundingClientRectangle.RelativeCenter.X),
                    (int)Math.Ceiling(toOffsetY ?? to.BoundingClientRectangle.RelativeCenter.Y))
                .Release()
                .Build()
                .Perform();
            ...
            for (int i = 0; i < nativeElements.Length; i++)
            {
                ...
                operation(
                    Actions.MoveToElement(
                        nativeElement,
                        (int)Math.Ceiling(relativeX ?? element.BoundingClientRectangle.RelativeCenter.X),
                        (int)Math.Ceiling(relativeY ?? element.BoundingClientRectangle.RelativeCenter.Y)),
                    nativeElement)
                .Build()
                .Perform();
            }
